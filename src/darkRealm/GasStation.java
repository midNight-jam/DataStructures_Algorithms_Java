...
//  Therefore, you can't travel around the circuit once no matter where you start.
  ...
  public static int canCompleteCircuit(int[] gas, int[] cost) {
    ...
    for (int i = 0; i < gas.length; i++) {
      ...
      // if at any point we loose all of our fuel, we make a fresh start at the next station, thus reset of 0 & start change
      if (tankFuel < 0) {
        tankFuel = 0;
        start = i + 1;
      }
    ...
    if (sum < 0) return -1; // not possible to make a round trip
