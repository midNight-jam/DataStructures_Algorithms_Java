...
//  importance value 15, 10 and 5, respectively. Then employee 1 has a data structure like [1, 15, [2]],
...
//  The maximum number of employees won't exceed 2000.
  ...
  public static int getImportance(List<Employee> emps, int id) {
    if (emps == null || emps.size() < 1) return 0;
    int res = 0;
    ...
    while (stack.size() > 0) {
      Employee trav = stack.pop();
      res += trav.importance;
      for (int i = 0; i < trav.subordinates.size(); i++) {
        stack.push(map.get(trav.subordinates.get(i)));
      }
    }
